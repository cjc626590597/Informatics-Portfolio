...
from website.database.models import testData
...
for i in range(0, len(dataList)):
    post = testData(Phone_name=dataList[i][0], Phone_price=dataList[i][1], Phone_factory_system_kernel=dataList[i][2],
                    Phone_screen_size=dataList[i][3], Phone_OS=dataList[i][4], Phone_resolution=dataList[i][5]
                    , Phone_frequency=dataList[i][6], Phone_kernel_num=dataList[i][7],
                    Phone_RAM_capacity=dataList[i][8],
