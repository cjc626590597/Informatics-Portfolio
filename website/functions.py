...
def main():
    baseurl = "https://detail.zol.com.cn/1338/"
    #1. 爬取网页
...
def getData(baseurl):
    datalist = []
    for i in range(1337760,1337770,1): #一次性检索不同的url，不同的url代表不同的手机
        url = baseurl + str(i) + "/param.shtml"
        html = askURL(url)    #保存获取到的网页源码
        #     # print(html)

        #获取种类为手机的网页
        if not isPhoneUrl(html):
            continue
...
def isPhoneUrl(html):
    bs = BeautifulSoup(html, "html.parser")
    if bs.findAll(class_="logo__classify")[0].text == '手机':
        return True
    else:
        return False
